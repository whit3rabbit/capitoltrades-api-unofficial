...
<task type="auto">
  ...
  <done>
    employer_mapping.rs exists with normalize_employer, match_employer (with configurable threshold), load_seed_data, is_blacklisted functions. 17 unit tests pass. strsim and toml dependencies compile.
  </done>
</task>
...
<task type="auto">
  ...
  <action>
    ...
    Note on scope: REQ-v1.2-009 references "top 200 employers" as an aspirational target from the research phase. For the initial seed file, we deliberately start with ~50 high-confidence mappings across key sectors. This is a practical decision: curating 200 employer-to-ticker mappings with verified name variants is time-intensive manual work that provides diminishing returns beyond the top contributors. The export-review-import workflow (Plan 03) is designed to grow the mapping database incrementally from real data. Starting small with high quality is preferable to padding with uncertain mappings.
...
- match_employer respects blacklist, short-name restriction, and configurable threshold
