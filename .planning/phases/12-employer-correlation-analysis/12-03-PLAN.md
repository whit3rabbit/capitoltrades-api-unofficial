...
must_haves:
  truths:
    ...
    - "Seed data is persisted to both employer_mappings and employer_lookup tables via load-seed subcommand"
    - "Missing tickers in seed data are skipped with a warning (user may not have synced that issuer)"
...
<task type="auto">
  ...
  <done>
    map-employers command registered with 3 subcommands. Export generates CSV with suggestions. Import reads confirmed CSV. Load-seed bootstraps from TOML and populates both employer_mappings and employer_lookup tables. All compile and pass clippy.
  </done>
</task>
...
- map-employers load-seed populates both employer_mappings and employer_lookup tables
- map-employers load-seed respects --dry-run flag
- Missing tickers in seed data produce warning and are skipped
</verification>
...
- Load-seed bootstraps database from TOML seed data into both employer_mappings and employer_lookup
