...
- Build `HashMap<(String, NaiveDate), Vec<usize>>` where values are indices into the trades vec.
...
- Create ProgressBar for unique (ticker, date) pairs count.
- Use Semaphore + JoinSet + mpsc pattern from sync.rs.
- Each spawned task: acquire permit, sleep for jittered delay `rand::thread_rng().gen_range(200..500)` ms, call `yahoo.get_price_on_date_with_fallback(&ticker, date).await`, send result on channel.
...
- Receiver loop: On Ok(Some(price)) -- for each trade index, call `parse_trade_range()` and `estimate_shares()` from the pricing module, then `db.update_trade_prices()`. Record success. On Ok(None) -- mark as enriched with None values (invalid ticker). On Err -- log warning, increment failed counter, record circuit breaker failure. Check breaker, abort_all if tripped.
...
- Receiver loop: On Ok(Some(price)) -- for each trade index, call `db.update_current_price(tx_id, Some(price))`. On Ok(None) or Err -- skip (current price is best-effort per research recommendation).
...
- The `trades` vec must be accessible from the receiver loop to look up PriceEnrichmentRow fields by index. Keep it as a local variable; the spawned tasks only send indices, not the full rows.
