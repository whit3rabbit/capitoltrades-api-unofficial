...
1. In `schema/sqlite.sql`, add `enriched_at TEXT` as the last column (before the closing paren or FOREIGN KEY clauses) in three CREATE TABLE definitions:
   - `trades` table: add `enriched_at TEXT,` after the `comment TEXT,` line and before the FOREIGN KEY lines
   - `politicians` table: add `enriched_at TEXT` after `chamber TEXT NOT NULL` (last column, before closing paren)
   - `issuers` table: add `enriched_at TEXT` after `sector TEXT` (last column, before closing paren)
...
4. Also add an index for enrichment queries (append to sqlite.sql):
   ```sql
...
- sqlite.sql has three new enrichment indexes
