...
<task type="auto">
  <name>Task 1: Fix upsert sentinel protection in all 4 upsert functions</name>
  <files>capitoltraders_lib/src/db.rs</files>
  <action>
Fix the ON CONFLICT DO UPDATE clauses in all upsert functions. Reference the column-by-column fix map from the research document. The changes are:

**A. upsert_trades (API path, ~line 82)**

1. Asset upsert (~line 86-93): Change `asset_type = excluded.asset_type` to:
   ```sql
   asset_type = CASE
...
2. Trade upsert (~line 136-184): Change the ON CONFLICT clause:
   - `has_capital_gains = excluded.has_capital_gains` becomes:
     ```sql
     ...
     END
     ```
   - `price = excluded.price` becomes `price = COALESCE(excluded.price, trades.price)`
   - `size = excluded.size` becomes `size = COALESCE(excluded.size, trades.size)`
   - `size_range_high = excluded.size_range_high` becomes `size_range_high = COALESCE(excluded.size_range_high, trades.size_range_high)`
   - `size_range_low = excluded.size_range_low` becomes `size_range_low = COALESCE(excluded.size_range_low, trades.size_range_low)`
   - `filing_id = excluded.filing_id` becomes:
     ```sql
     filing_id = CASE
     ...
     END
     ```
   - `filing_url = excluded.filing_url` becomes:
     ```sql
     filing_url = CASE
     ...
     END
     ```
   - Add at the end of the SET clause: `enriched_at = trades.enriched_at`

3. Politician upsert in upsert_trades (~line 105-134): Add at the end of the ON CONFLICT SET clause:
   `enriched_at = politicians.enriched_at`

4. Issuer upsert in upsert_trades (~line 94-103): Add at the end of the ON CONFLICT SET clause:
   `enriched_at = issuers.enriched_at`

**B. upsert_scraped_trades (scraper path, ~line 272)**

5. Asset upsert (~line 276-282): Same fix as A.1 above (asset_type CASE expression).

6. Issuer upsert (~line 284-293): Add `enriched_at = issuers.enriched_at` to the ON CONFLICT SET clause.

7. Politician upsert (~line 295-325): Add `enriched_at = politicians.enriched_at` to the ON CONFLICT SET clause.

8. Trade upsert (~line 327-375): Apply the SAME changes as A.2:
   - has_capital_gains: CASE expression (keep existing if incoming = 0)
   - price: COALESCE(excluded, existing)
   - size: COALESCE(excluded, existing)
   - size_range_high: COALESCE(excluded, existing)
   - size_range_low: COALESCE(excluded, existing)
   - filing_id: CASE expression (keep existing if incoming > 0)
   - filing_url: CASE expression (keep existing if incoming != '')
   - Add: `enriched_at = trades.enriched_at`

**C. upsert_politicians (~line 443)**

9. Politician upsert: Add `enriched_at = politicians.enriched_at` to the ON CONFLICT SET clause. Do NOT change any other columns -- this function receives complete data, so unconditional overwrite is correct for all fields except enriched_at.

**D. upsert_issuers (~line 590)**

10. Issuer upsert (~line 594-603): Add `enriched_at = issuers.enriched_at` to the ON CONFLICT SET clause. Do NOT change other columns -- same reasoning as politicians.
...
Manually inspect the 4 upsert functions in db.rs to confirm:
...
- price, size, size_range_high, size_range_low use COALESCE
  ...
  <done>
- All 4 upsert functions have sentinel-protected columns via CASE expressions
- All 4 upsert functions have COALESCE for nullable columns (price, size, size_range_*)
- All 6 ON CONFLICT clauses (2 in upsert_trades, 3 in upsert_scraped_trades, 1 in upsert_politicians, 1 in upsert_issuers, plus 2 asset upserts) preserve enriched_at
...
<verification>
After both tasks, verify ALL four phase success criteria from ROADMAP.md:
...
- enriched_at preserved in ALL 6 ON CONFLICT clauses across 4 upsert functions
