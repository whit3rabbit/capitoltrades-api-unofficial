...
must_haves:
  truths:
    - "This plan provides calculation primitives and DB access layer only -- it does NOT execute enrichment, validate tickers against Yahoo, or run batch processing (those belong to Phase 4)"
    - "Dollar range bounds are extracted from size_range_low/size_range_high columns; returns None when either bound is missing (no fallback to value column)"
    ...
    - "DB queries JOIN the issuers table to access issuer_ticker (ticker lives on issuers, not trades)"
  artifacts:
  ...
  key_links:
    ...
    - from: "capitoltraders_lib/src/db.rs"
      to: "issuers table"
      via: "All three DB methods JOIN issuers to access issuer_ticker (ticker is NOT on trades table)"
      pattern: "JOIN issuers.*ON.*issuer_id.*issuer_ticker"
---

<objective>
Implement dollar range parsing, share estimation logic, and database operations for price enrichment.

Purpose: Phase 3 provides the calculation layer and data access layer that Phase 4's enrichment pipeline will orchestrate. This plan does NOT validate tickers or run batch processing -- ticker validation happens implicitly in Phase 4 when YahooClient.get_price_on_date() returns Ok(None) for invalid/delisted tickers. Without this plan, the Phase 4 pipeline has no way to estimate share counts from dollar ranges or persist price data.
...
- If either is None, return None -- estimation requires both bounds to validate the result against the original range (REQ-E4 success criterion 4). There is no fallback to the `value` column.
- Return None if low > high (invalid data) or if both are zero

Note: The previous signature included a `value: i64` parameter. This has been removed because the function never uses it -- when bounds are missing, we return None rather than falling back to value. The `value` field on PriceEnrichmentRow still exists for potential future use but is not consumed by parse_trade_range.
...
**IMPORTANT: issuer_ticker lives on the `issuers` table, NOT the `trades` table.** All three DB methods below MUST JOIN `issuers` on `t.issuer_id = i.issuer_id` to access `i.issuer_ticker`. Forgetting this JOIN will cause column-not-found errors or silently wrong results.
...
- CRITICAL: Must JOIN issuers -- issuer_ticker is on issuers table, not trades table.
...
- CRITICAL: Must JOIN issuers -- issuer_ticker (`i.issuer_ticker`) comes from the issuers table.
...
- Note: update_trade_prices does NOT need a JOIN -- it updates trades by tx_id directly.
...
5. count_unenriched_prices and get_unenriched_price_trades both JOIN issuers to access issuer_ticker
6. update_trade_prices always sets price_enriched_at (resumability)
7. parse_trade_range returns None when either bound is missing (no value fallback)
</verification>
...
- DB queries JOIN issuers table to access issuer_ticker
